...
import static org.docx4j.convert.in.xhtml.HtmlToDocConfig.PARAGRAPH_SPACING_AFTER_LINES;
import static org.docx4j.convert.in.xhtml.HtmlToDocConfig.PARAGRAPH_SPACING_AFTER_POUNDS;
import static org.docx4j.convert.in.xhtml.HtmlToDocConfig.PARAGRAPH_SPACING_BEFORE_LINES;
import static org.docx4j.convert.in.xhtml.HtmlToDocConfig.PARAGRAPH_SPACING_BEFORE_POUNDS;
import static org.docx4j.convert.in.xhtml.HtmlToDocConfig.PARAGRAPH_SPACING_LINE_VALUE;
...
public class XHTMLImporterImpl implements XHTMLImporter {
    ...
    public XHTMLImporterImpl(WordprocessingMLPackage wordMLPackage) {
        ...
        addFonts(HtmlToDocConfig.FONTS_ZH_CN);
    }

    private void addFonts(String... fonts) {
    ...
    public Map<String, CSSValue> getCascadedProperties(CalculatedStyle cs) {
        ...
        for (int i = 0; i < derivedValues.length; i++) {
            ...
            if (name.toString().startsWith("-fs"))
                continue;
    ...
    private P getCurrentParagraph(boolean create) {
        if (attachmentPointP != null)
            return attachmentPointP;
        if (create) {
    ...
    private ContentAccessor getListForRun() {
        ...
        if (attachmentPointH != null)
            return attachmentPointH;
    ...
    private void prepareCaption(Element figcaption, P currentP) {
        ...
        if (!sequenceName.endsWith(" "))
            sequenceName += " "; // adds trailing space
    ...
    private void addImage(BlockBox box) {
        ...
        Long cy = contentBounds.height == 0 ? null : UnitsOfMeasurement.twipToEMU(dotsToTwip(contentBounds.height));
        Long cx = contentBounds.width == 0 ? null : UnitsOfMeasurement.twipToEMU(dotsToTwip(contentBounds.width));

        xHTMLImageHandler.addImage(renderer.getDocx4jUserAgent(), wordMLPackage, this.getCurrentParagraph(true), box.getElement(), cx, cy);
    }
    ...
    private void processInlineBox(InlineBox inlineBox) {
        ...
        if (s.getElement() != null
            ...
            CTMarkupRange inlineMarkupRange = bookmarkHelper
                .anchorToBookmark(s.getElement(), bookmarkNamePrefix, getCurrentParagraph(false), this.contentContextStack.peek());
        ...
        if (p.getPPr() == null) {
            ...
            populatePPr(pPr, s,
                cssMap);  // nb, since the element is likely to be null, this is unlikely to give us a @class; that's why we need to make the p in the enclosing div (or p).
    ...
    private void setParagraphSpacing(PPr pPr) {
        PPrBase.Spacing spacing = pPr.getSpacing();
        if (spacing == null) {
            spacing = new PPrBase.Spacing();
            pPr.setSpacing(spacing);
        }
        String before = ImportXHTMLProperties.getProperty(PARAGRAPH_SPACING_BEFORE_POUNDS, "0");
        String after = ImportXHTMLProperties.getProperty(PARAGRAPH_SPACING_AFTER_POUNDS, "100");
        String beforeLines = ImportXHTMLProperties.getProperty(PARAGRAPH_SPACING_BEFORE_LINES, "200");
        String afterLines = ImportXHTMLProperties.getProperty(PARAGRAPH_SPACING_AFTER_LINES, "0");
        String lineValue = ImportXHTMLProperties.getProperty(PARAGRAPH_SPACING_LINE_VALUE, "360");
        ...
        if (StringUtils.hasText(beforeLines)) {
            // 段前行数
            spacing.setBeforeLines(new BigInteger(beforeLines));
        }
        if (StringUtils.hasText(afterLines)) {
            // 段后行数
    ...
    private PPrBase.Ind getParagraphInd(PPr pPr) {
